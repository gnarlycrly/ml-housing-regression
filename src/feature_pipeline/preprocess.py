...
def base_name(name: str) -> str:
    """Return metro name before comma (drop state), like in the notebook."""
    if pd.isna(name):
        return name
...
def clean_and_merge(df: pd.DataFrame, metros_path: str | None = "data/raw/usmetros.csv") -> pd.DataFrame:
    """
    Normalize / fix city names and merge lat/lng from metros dataset.

    Reproduces prior notebook behavior:
    - Apply CITY_MAPPING on city_full
    - Build metro_key from city_full and metro_full (drop ", State")
    - Merge on metro_key to get lat/lng
    """
    ...
    # 1) Fix city names exactly like before
    df["city_full"] = df["city_full"].replace(CITY_MAPPING)

    # 2) If lat/lng already present, don't touch
    if {"lat", "lng"}.issubset(df.columns):
        print("⚠️ Skipping lat/lng merge: already present in DataFrame.")
        return df

    # 3) If no metros file provided / exists, skip merge
    ...
    # 4) Create metro_key on both sides (drop state part)
    df["metro_key"] = df["city_full"].apply(base_name)
    metros["metro_key"] = metros["metro_full"].apply(base_name)

    # 5) Merge lat/lng using metro_key
    df = df.merge(
    ...
    # 6) Check for any cities that still didn't get lat/lng
    ...
    df.drop(columns=["metro_key"], inplace=True, errors="ignore")
    return df
